...
$fin_year_end=fullclean($_POST['fin_year_end']);
$chk1=($_POST["submit"]=="Save")?"Pending":"Submit";
try {
$stm = $conn1->prepare("INSERT INTO `project_monthly_report`( `project_id`, `month`,`fin_year_end`, `budget_utilized`, `progress_till_date`, `chk1`) VALUES (:pid,:mn,:fye,:bu,:ptd,:chk)");
$stm->bindParam(":pid", $pid, PDO::PARAM_INT);
$stm->bindParam(":mn", $month, PDO::PARAM_INT);
$stm->bindParam(":fye", $fin_year_end, PDO::PARAM_INT);
$stm->bindParam(":bu", $budget_u, PDO::PARAM_INT);
$stm->bindParam(":ptd", $progress_till, PDO::PARAM_STR, 2000);
...
$year = date("Y");
try {
	$stm = $conn1->query("SELECT sno FROM project_monthly_report WHERE project_id = '$pid' and month = $month and YEAR(time) = $year");
	$rows = $stm->fetch();
} catch(PDOException $e) {
...
elseif($_POST["submit"]=="Submit"){ ?>
	<form action="monthlyreport.php?EncHid=<?php echo $_SESSION['EncTok'] ?>" method="POST" name="myForm">
