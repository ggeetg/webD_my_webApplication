...
$other_expnse=fullclean($_POST['o_exp']);
...
$stm = $conn1->prepare(" INSERT INTO `acr`(`mr_activity_id`, `target_group`, `bud_san_no`, `bud_san_date`, `bud_amnt`, `sc_st_group`, `objective`, `brief`, `difficulties`, `amnt_spent`, `venue`, `start_date`, `end_date`, `outcome`, `impact`, `measures`, `followup`, `faculty`, `total_participants`, `total_external_resource`, `total_internal_resource`, `contingency`, `othr_expense`, `chk1`, `completion_fy_end`) VALUES (:a,:b,:c,:d,:e,:f,:g,:h,:i,:j,:k,:l,:m,:n,:o,:p,:q,:r,:s,:t,:u,:x,:y,:v,:w) ");
...
$stm->bindParam(":w", $fy_year, PDO::PARAM_INT);
	$stm->bindParam(":x", $contingency, PDO::PARAM_INT);
	$stm->bindParam(":y", $other_expnse, PDO::PARAM_INT);
$stm->execute();


$stm = $conn1->query("select id from acr where mr_activity_id = $mr_id ");
$row = $stm->fetch();
...
for($i=0;$i<2;$i++){
//$acrid=!empty($acrid[$i])?$acrid[$i]:null;
...
$t_expend[$i]=!empty($t_expend[$i])?fullclean($t_expend[$i]):null;
	...
	$stm = $conn1->prepare(" INSERT INTO `acr_expenditure`(`acr_id`, `participants_category`, `number`, `ta`, `honorarium`, `total_expenditure`) VALUES (:a,:b,:c,:d,:f,:h) ");
			...
			if(in_array(strtolower($file_ext), $allowed_types)) {
				...
				if(file_exists($filepath)) {
					...
					if( move_uploaded_file($file_tmpname, $filepath)) {
						...
						$stm->bindParam(":c", $acrid, PDO::PARAM_INT);
						$stm->execute();
					}
					else {					
