...
<!--===============================================================================================-->
  ...
  function validate(){
    if($("#fname").val()==""||$("#lname").val()==""||$("#email").val()==""||$("#dept").val()==""||$("#role").val()==""||$("#pfx").val()==""||$("#desig").val()==""||$("#phn").val()==""||$("#ustatus").val()==""){
...
<div class="container p-r-25 p-l-25 p-t-45 p-b-45">
  ...
  <form action="edituseraction.php?EncHid=<?php echo $_SESSION['EncTok'] ?>" method="post" onsubmit="return validate()">
    ...
    <table class="table">
      ...
      <tr>
        ...
        <td>
          <div class="row">
            <div class="col">
              <input type="password" class="form-control b2" placeholder="Enter New Password" id="passcode" name="passcode" minlength="6" style="background: deeppink; color: #fff;" data-toggle="tooltip" title="Should be at least 6 digit long and contains at least one special character" autocomplete="off" readonly onfocus="this.removeAttribute('readonly');">
              <span id="err1"></span>
            </div>
            <div class="col d-flex align-items-center">
              <input type="checkbox" name="f_reset" id="f_reset" value="1">
              <label for="f_reset" class="ml-1">Force Reset</label>
            </div>
          </div>
        </td>
      </tr>
      ...
      <tr>
        <td>User Status</td>
        <td><select name="ustatus" id="ustatus" class="form-control"><option value="Active">Active</option><option value="Deactivate">Deactivate</option></select></td>
      </tr>
    </table>
...
<script type="text/javascript">
  <?php
    $uid = $_POST['submit'];
    $stm = $conn1->query("SELECT a.id, a.uname, a.upass, a.utype, a.name_prefix, a.first_name, a.last_name, a.dept, a.designation, a.desig_other, a.contact, a.dept_code, a.head_dept, a.head_dept_code, a.email, a.menu_ref_id, a.status FROM user as a WHERE a.id = $uid");
    $row = $stm->fetch();
  ?>
  document.getElementById("uid").value="<?php echo $uid ?>";
  ...
  document.getElementById("dept").value="<?php echo $row['dept'] ?>";
  ...
  document.getElementById("role").value="<?php echo $row['utype'] ?>";
  document.getElementById("ustatus").value="<?php echo $row['status'] ?>";
