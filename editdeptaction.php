...
if (true) {
    ...
    if (! empty($csrfResponse)) {
		...
		echo "<script>location.href='editDept.php?EncHid=".$_SESSION['EncTok']."&msg=".urlencode("Someting went wrong, please try again later.")."';</script>";
		exit();
    }
}
?>
...
function validate_input(){
	...
	if($_POST['dept_status']==""){
			$flag=0;
			$flag_msg.=' , department status ';	//
	}

	if($flag==0){
		echo "<script>location.href='editDept.php?EncHid=".$_SESSION['EncTok']."&msg=".urlencode("Please fill the following fields:-".$flag_msg.".")."';</script>";
		exit();
	}
}
validate_input();
...
$sts=fullclean($_POST['dept_status']);
$id=fullclean($_POST['dept_id']);

try{
$stm = $conn1->prepare("UPDATE `department` SET `dept_name`=:a, `short_name`=:b, `dept_pac_code`=:c, `status`=:d where id=:e ");
...
$stm->bindParam(":e", $id, PDO::PARAM_INT);
$nrow = $stm->execute();
